package com.march.reaper.widget.refresh;
...
//            Log.e("chendong","mRefreshTargetOffset  = " + mRefreshTargetOffset
//                    +"   ok = " +((targetEnd - mFrom) * interpolatedTime)
//                    + "  ,toRefresh from = " + mFrom + "  , targetTop = " + targetTop +
//                    "  ,rst = " + (-mCurrentScrollOffset - targetTop));
    ...
    // 当RecyclerView开始进行嵌套滑动会先询问父View是否要消耗，
    //
    @Override
    ...
    //未到达顶部时开始拖动直到到达顶部继续拖动，此时RecyclerView不进行处理，会告诉parent有多少没有消耗，
    //返回的dyUnConsumed不为0，RecyclerView在自己区域内滑动时会全部消耗，返回的dyUnConsumed＝0
    //为了实现刷新的效果，开始按照RecyclerView继续滑动的距离移动头部

    @Override
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed, int dxUnconsumed,
                               int dyUnconsumed) {
        Log.e("chendong","onNestedScroll(dyUnconsumed = "+ dyUnconsumed +" ,  mTotalUnconsumed = " +mTotalUnconsumed);
        if (dyUnconsumed < 0) {
            Log.e("chendong","onNestedScroll(dyUnconsumed = "+ dyUnconsumed +" ,  mTotalUnconsumed = " +mTotalUnconsumed);
